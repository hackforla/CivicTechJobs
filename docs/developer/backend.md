...
Same-origin in dev / stage / prod (Next rewrites proxy `/api/*` and `/admin/*` through the Next runtime to Django; see [next.config.ts](https://github.com/hackforla/CivicTechJobs/blob/develop/frontend/next.config.ts)) means cookies always apply and there's no CORS surface anywhere.
